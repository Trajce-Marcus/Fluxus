...
└─────────────┬─────────────┘                        │
       ┌──────▼─────────────┐                        │
       │  @fluxus/client    │────────────────────────┘
       │  config + pages    │  config.getForOperation (runtime)
       │  snapshot; records │  config.get (design) · records.partition
       │  all, or on demand │  /list/get · pages.* · activities.run
       └────────────────────┘
```
...
**One library may import another, and one does:** `@fluxus/workbench` → `@fluxus/page-runtime` for the standard capture form (2026-08-16). Running a UI activity is one form, and it belongs with the smaller, more general cluster; the reverse direction would have made the page runtime depend on the whole record UI to draw one dialog. What the form needs from whichever host opened it — evaluate an expression, reach a GET, upload a file, resolve a reference's label, and optionally a record picker — is the injected `CaptureHost`. The apps-never-import-apps rule is untouched.
...
Since 2026-08-09 that pipeline has a **read half**: a GET activity (`record_map: "GET"`) shares the availability gate and the before hook, then answers with its `returns` expression instead of persisting (`engine.runQuery`, `activities.query`, `client.query()` — [DATA_THROUGH_ACTIVITIES](DATA_THROUGH_ACTIVITIES.md) step 1). It matters because the query then lives **in the model**: an app names an activity, and `returns` never reaches the browser. **Since 2026-08-10 an app actually does** (step 2): a page's dynamic prop names a GET with `invoke(...)` in the expression it already was, so a page carries the activity's name where it used to carry the query. **Since 2026-08-11 reads are logged** (step 3): a GET records one light entry on its anchor record — parameters, caller, outcome, duration, never the answer — and a page gets an anchor by declaring the record it is about, found or created at page open. That closes the last part of "the pipeline is the log" that reads did not keep. The three doors below are the write half.
...
3. **Headless invocation** — the activity's attribute list *is* its parameter signature; callers supply values in one payload; datasources double as validation. *(Live — `@fluxus/server`, backend stage 1, 2026-07-12: `activities.run` over tRPC. Since backend stage 2 — same day — the browser hosts run through this same door via `@fluxus/client`: one model, many apps, literally. Since 2026-09-25 ([SERVER_DATA_LOADING](SERVER_DATA_LOADING.md) step 2) the server runs the engine over a **database store** that reads each record when a script needs it and writes each change as it happens, inside one transaction per activity run with a savepoint per hook; until then it loaded the whole operation into memory per request and wrote the diff back.)*
...
- **The server reads only what a request touches** ([SERVER_DATA_LOADING](SERVER_DATA_LOADING.md); BLUEPRINT § Loading only what a request needs). The engine runs against a database store that asks Postgres for a record by id, a type's records, or records by a field value when a script needs them — never a record's history — and holds what it read for the rest of the request only. Every write goes to the database as it happens, inside the run's transaction, so the database itself shows a run its own changes. *Built 2026-09-25 as step 2; step 3 — record queries run as SQL — is not built, so `records.x.where(...)` still reads the whole of type `x` and filters it in memory.*
  - *Reversed 2026-09-25: **partition-fetch + filter.*** Runtime DSL queries were answered by loading the operation's partition — every record, with full histories — into memory for each run and each GET, and filtering; "leanness of the transactional layer" was load-bearing, and retention (`complete_when` and a window per record type, archiving completed records; tiering the history of never-ending records) was what was to keep partitions small. Dropped because the cost grew with the operation rather than with the request — it failed at 1,429 records, a page with eight lists doing the whole load eight times — and no retention policy bounds an operation holding millions of live records. Retention stands for its legal and compliance purpose, no longer as the runtime's speed guarantee.
- **A third table family sits outside both layers: the performance log** (`perf_spans`, `perf_settings`; built 2026-09-25, [PERFORMANCE_LOGGING.md](PERFORMANCE_LOGGING.md)). It is timing about the platform, not data about the business — never a record's history, never projected to reporting, deleted after 30 days, and safe to wipe. Nothing reads it back into a run.
...
| | **DynamoDB single-table** — the scale option | On-demand pricing, effectively unbounded throughput, Streams as the built-in change feed. Key-based access only — so, with record queries running as SQL (SERVER_DATA_LOADING step 3), a DynamoDB store would answer less than Postgres does; the seam (`WaitingStore`) survives. |
...
The engine package's `Store` contract and the DSL's `RecordsHost` are the seams all of this hides behind. Drizzle ORM over Postgres for schema and queries; tRPC for the function-call API surface (no GET/POST distinction — pages and headless callers just call functions by name). **The Store is split** (2026-09-25, SERVER_DATA_LOADING step 2): the **model** half answers immediately; the **records** half may answer with a promise; `subscribe` is the browser's alone. The browser keeps an immediate `Store` (the `MemoryAdapter`), so nothing there handles a promise; the server's `DatabaseStore` is a `WaitingStore` that talks to Postgres per read and per write. The DSL evaluator is one implementation run two ways — an **immediate** driver over memory (the browser, unchanged) and a **waiting** driver that awaits the database, services and `invoke` (the server) — and the engine's server-only functions (`runActivity`, `runQuery`, `invoke`, `validateSubmission`) wait. *Reversed: the synchronous Store (ruled at Phase 4)*, which kept the server from talking to Postgres through a Store at all — it snapshotted the partition into memory per request and wrote the diff back, the partition-fetch model made literal. Dropped with that model, for the same reason.
