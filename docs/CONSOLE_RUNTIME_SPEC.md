...
Status: **rev 7 (2026-07-27) — M1–M15 BUILT.** M15 completes the split the last
...
- `orgs(id, name, contact_email, plan, status, created_at)` — the tenant (M13 name, M14 profile). `operations.org_id` / `role_assignments.org_id` have carried an org key since M1 with nowhere to read a row from; this is that row. `created_at` is the registration date, `plan` the subscribed tier (default `'free'`), `status` `'active'`. One `'default'` row seeded by the migration; no signup path.
...
- **Two-level IA (M7; chrome restandardised M16)**: the Console is a *workspace* until a solution is opened.
  - **Workspace level** (no solution open) — cross-solution/org admin: **Organisation** (the M14 settings surface — profile + plan; first item), Solutions (list/create + operation count; **Open** = design it here), Operations (**Open** = run it in the Runtime app; **Admin** = the operation view, M11), Implementer levels. Since M11 the operation-scoped admin — menu override, role assignments — lives inside the **operation view** (master/detail under Operations), not as separate picker-driven panels; the Operation menu and Role assignments sidebar entries retired.
  - **Solution level** (a solution open) — that solution's design artifacts: **Overview** (landing), **Pages** (the builder), the **SDM group** (Record types, Attributes, Workflows, Roles, Menu), **Workbench** (M15 — the record/activity surface, against the M9 data operation).
...
- **Console shell (M16, ruled 2026-07-30 — industry-console chrome, Neon-style)**: the VS Code frame (activity bar + secondary sidebar + global tab strip) retired — it is a document-editor idiom, and only the page editor is a document editor. Replaced by:
  - **Breadcrumb header**: `Fluxus · <org>` always; in solution scope ` / <solution> ▾` — the crumb is a dropdown listing the org's solutions (switch from anywhere) plus **All solutions** (back to workspace). The **Data** picker (M9) stays on the header's right in solution scope. The dead workspace search field retired.
  ...
  - **Inner panels belong to sections, not the shell.** Record types / Workflows etc. draw their own master/detail inside their page; **Pages** is the one document-editor section and owns the page tree, the open-page **tabs**, the Components/Search panels and the bottom console — all folded inside `PagesSection`, out of shell state's meaning (the shell's `openTabs`/`activeTab` now mean page-editor tabs only).
  - **Hash routes**: `#/<section>` (workspace), `#/s/<solutionId>/<section>` (solution scope) — location legible, bookmarkable, browser back works. One-way sync state→hash via store subscription; hash→state on `hashchange`/boot (a solution route boots via `openSolution` + a name lookup from `solutions.list`). A section's `canLeave` guard (the SDM dirty-draft confirm) runs on any navigation away, nav click or hash.
...
- **The Runtime app renders published pages only; the workbench leaves it** (ruled 2026-07-27, superseding the 2026-07-20 "stays reachable for now"). The line is *runtime = the authored app, Console = model + data tooling*: raw record access, running any activity, CSV import and the schema navigator are implementer work, and Console already connects against a data operation (M9), so the workbench lands there next to Pages and SDM and the SDM editor stops authoring blind. Consequences: no menu-addressable "Workbench" item, no record-type list in the Runtime nav, and an operation whose solution has no published pages shows an empty app — the escape hatch is gone by design. **BUILT 2026-07-27 as M15** (§9).
- **The workbench becomes one component** (ruled 2026-07-27) — `<Workbench client user? />` (as built; the `client config` first sketched was redundant — config, adapter and uploads all hang off the client) owning its own context (selected record type, record, activity in flight, CSV/schema-nav state); nothing above it knows those exist. Today's `AppContext` mixes shell state with record state, and that mixing *is* the coupling: the split — session/user + operation + effective menu + current page in a `RuntimeContext`, everything record-shaped inside the workbench — is the extraction. It stays inside `@fluxus/sdm` until a second host wants it; a package needs a name endorsement. Orthogonal to and non-blocking for the deferred `RecordGrid`/`RecordView` page-runtime decomposition (§10) — this is the container step, that is the inner one. **BUILT as M15** — signature
  `<Workbench client user? />`, cluster at `packages/sdm/src/workbench/`, hosted by
  Console's `WorkbenchView`; see §9 M15.
- **Solution branding, not platform branding** (cosmetic MVP; no branding config yet — logo/colours deferred, §10). The header carries an **identity line** (M13): **org name · solution name** on the left — who you work for, which app you are in — the **operation name** as a context chip on the right, then the user menu. Platform attribution is one **"Powered by Fluxus"** line at the foot of the nav, deliberately at the edge. All three names arrive on the connect snapshot (`operations.get`). The operation chip is **display-only** — switching operations in-session needs a memberships list and is not built.
