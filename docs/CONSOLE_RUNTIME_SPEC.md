...
- **SDM editor** (solution level, M7): model authoring — record types (+ custom fields, workflow binding, RBAC read surface), the attribute pool (key/label/type + fk/list/multi/multiline config), and role defs. Plain forms over `config.get`/`config.put`; a save persists the whole config, reloads the solution to rebuild the model, and remounts. Workflow/activity/hook (FluxScript) authoring stays hand-edited — deferred slice.
