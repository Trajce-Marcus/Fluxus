...
- **Two-level IA (M7)**: the Console is a *workspace* until a solution is opened.
  ...
  - **Solution level** (a solution open) — that solution's design artifacts: **Pages** (the builder), **SDM** (model editor), **Workbench** (M15 — the record/activity surface, against the M9 data operation), Components, Search. Activity bar switches to this set; header shows the solution name + **← Solutions** to return. The Workbench item is the one bar item that opens a content tab directly instead of a sidebar panel — it is a surface, not a browser.
