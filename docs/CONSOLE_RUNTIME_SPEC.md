...
Status: **rev 6 (2026-07-27) — M1–M14 BUILT; M15 ruled, not built (workbench out of Runtime, into Console).** M13 gives the Runtime app its identity
line (org · solution … operation · user, Powered by Fluxus) over a new `orgs`
table, and rules that the Runtime app renders published pages only. M14 makes
the org a real tier: an onboarded org registers, has a profile and a plan, and
its solutions/operations hang under it — Console gets an **Organisation**
settings surface and shows the org name at both IA levels. 75 server tests +
full workspace build green. Prior rev: M9 closes the design-plane data
...
- **The workbench becomes one component** (ruled 2026-07-27) — `<Workbench client config />` owning its own context (selected record type, record, activity in flight, CSV/schema-nav state); nothing above it knows those exist. Today's `AppContext` mixes shell state with record state, and that mixing *is* the coupling: the split — session/user + operation + effective menu + current page in a `RuntimeContext`, everything record-shaped inside the workbench — is the extraction. It stays inside `@fluxus/sdm` until a second host wants it; a package needs a name endorsement. Orthogonal to and non-blocking for the deferred `RecordGrid`/`RecordView` page-runtime decomposition (§10) — this is the container step, that is the inner one.
...
- **Shell (M10, header revised M13)**: standard app-shell chrome — top bar (the M13 identity line above, UAT toggle + notification bell retained, user menu w/ sign-out), collapsible left nav ending in the Powered-by-Fluxus line, content area. Nav is the effective menu (§5); when a menu is effective the record-type and pages sidebar listings retire from the nav (the pages listing remains solely as the **no-menu fallback**, so an unconfigured demo operation is unchanged — adoption posture). The M10 Workbench menu item and Workbench-scoped record-type list are retired by M15. Plain CSS; no component-library dependency without a ruling.
...
15. **M15 — workbench out of Runtime, into Console (UI phase step 6)**: **RULED 2026-07-27, not built.** Collapse the workbench cluster (`RecordTypeList`/`RecordsGrid`/`RecordView` + activity strip, capture form, history, schema navigator, CSV import) into one `<Workbench>` component with its own context; split `AppContext` into that and a `RuntimeContext` (session/user, operation, effective menu, current page); mount it as a Console solution-level activity against the M9 data operation; delete it from the Runtime app, leaving published pages only, and retire the M10 Workbench menu item. Stays inside `@fluxus/sdm` for now — a package extraction needs a name endorsement.
