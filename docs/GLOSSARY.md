...
- **Host (of the engine)** — an application that constructs an Engine (`createEngine({ store, config, services })`) and drives activities through it: the workbench, the page builder (Extraction stage 2), headless callers (Phase 4). Hosts own everything the engine deliberately doesn't: UI state, notification surfaces, warning channels, service module implementations.
