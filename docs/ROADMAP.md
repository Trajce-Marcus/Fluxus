...
| `@fluxus/client` | **Built at backend stage 2 (2026-07-12):** the browser hosts' door to the server — `connect()` fetches a scope's stored config + record partition into the engine's `MemoryAdapter` (UI reads and FluxScript evaluation stay local + synchronous), `runActivity` round-trips `activities.run` then re-fetches the partition. `@fluxus/server` is a type-only dependency; hard cutover — no localStorage fallback. |
