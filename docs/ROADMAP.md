...
DSL Phase 4 (headless invocation)                                ✅ stage 1 done
  └─► backend: activities as the API surface                     (2026-07-12)
      (tRPC + Hono/Lambda + Postgres per ARCHITECTURE.md)
      stage 2 open: browser hosts repoint from localStorage
      to the backend (Store swap becomes literal)
```
...
7. **Backend stage 2 — hosts repoint** — the workbench and page builder swap localStorage for the backend: partitions fetched via `records.*` (client-side expression evaluation stays local), activity runs through `activities.run` (hooks and persistence server-side only), making "one model, many apps" literal.
