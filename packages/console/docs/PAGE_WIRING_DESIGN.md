...
**Status: BUILT (2026-07-12) — decisions 1–7 implemented; SPEC.md carries the current truth.**

**Amended 2026-08-10** ([DATA_THROUGH_ACTIVITIES step 2](../../../docs/DATA_THROUGH_ACTIVITIES.md)): decision 2 stands and gains a second form. A dynamic prop is still one FluxScript expression — but that expression may **name a GET activity** rather than carry the query itself: `invoke('act_get_work_orders', { status: context.page.status })`. No storage change and no new binding shape; the structured `{ activity, params }` alternative was rejected precisely because it would have reinstated the dropdown-built union this document removed. The editor UI is unchanged (decision 6's dialog writes either form). Evaluation is now async, and `invoke` is available in a dynamic prop only — never in a callback.

**Amended 2026-08-11** (step 3): a page also declares **the record it is about** — `PageDef.record` (`{ type, instances: 'one' | 'many' }`), authored in the editor's new **Page Record** section beside Page Access. It is the anchor every run fired from the page is recorded against, including the GETs a dynamic prop names, and it arrives in the ctx root as `context.record`. Nothing about the wiring format changed; what changed is that the page now has something to anchor on.

**Amended 2026-08-09** ([DATA_THROUGH_ACTIVITIES §4](../../../docs/DATA_THROUGH_ACTIVITIES.md)): the callback payload's free-form `data` half is **removed**. Components emit one `value`, the payload packs as `callbackData.value` alone, and the signature is `services.activities.run(activityId, record)`. Read the two mentions below with that in mind — what an activity needs, it declares as an attribute.

Implementation notes: the ctx root keeps the DSL's canonical name `context` (this doc's `ctx` was shorthand); the callback payload packs as `callbackData.value` / `callbackData.data` (components emit `(value, data?)`); `show-overlay` was **cut** (the stub had no consumer); the validator gained a `callback` script mode (effects allowed, mutations rejected) and page callbacks run in `mutate` mode against a mutation-less records host; activity runs moved off `services.page` onto the host-neutral **`services.activities.run(activityId, record, data)`** (ruled 2026-07-12) — same manifest in every host, so activity-running scripts are portable to the Phase 4 headless host. **Not yet built** (tracked on the ROADMAP): the crossover acceptance bar (expression↔function traversal, promote-to-function), and the open questions below — including the callback error contract, so run outcomes still surface through the host (confirm dialog + page error banner), not as script return values.
