...
**`invoke(activityId, params?)`** is the built-in that runs one. It reaches
GET activities only, so it is read-only by construction and therefore legal
everywhere — expressions, before hooks included; a hook that guards a
submission by re-asking the model is the point (DATA_THROUGH_ACTIVITIES §3).
The language declares the built-in and nothing more: which ids exist is the
host's knowledge, supplied through `EvalHost.invoke`, and a host that runs no
...
- **~~No text → number cast, so stored money cannot be added up~~ — RESOLVED 2026-09-22.** The diagnosis was half right and the cause was in the wrong package. A `decimal` field did persist as text, but not because the language lacked a cast: `runActivity` wrote the **raw captured bag** into the record while coercing a separate copy for the hooks, so the number never reached storage. Fixed where every write goes through — `memoryAdapter.coerceFieldValues` — and the 18 stored values converted by `packages/server/scripts/numeric-fields-to-numbers.ts`. Stored numbers now add: the projects solution's total GETs, built and broken since 2026-09-14, return real figures without a line of script changing. **Still absent, and now genuinely language-level:** a `number(text)` builtin for text that is honestly text, and `sum` on the chain (the named-function `for each` loop stands in). See `solutions/projects/docs/PROJECT_PROFILE_PAGE.md` §8-5.
