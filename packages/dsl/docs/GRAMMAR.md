...
- **`delete()` follows every one of those rules** (2026-09-21): no argument, filter mandatory in bulk, projected rows refused, after hooks only, staged and rolled back with the rest. It destroys the record **and its history** — anything worth keeping is marked instead, the way a solution's own `expired` field does. Deleting a record the same script created cancels the create rather than staging a delete of a row the host has never seen, and an update staged against a record later deleted is dropped. On a write-through host (DSL_SPEC §7 — the server) nothing is staged: the create landed and the delete removes it, and what still points at a deleted record is checked when the script ends.
- Update-by-raw-id is `records.jobs.where(id = x).first.update({...})`; a `.get(id)` sugar is deferred until that chafes.
- All mutations run only in after hooks, are atomic with the rest of the hook — staged and committed together by a staging host, written through inside the hook's savepoint by a write-through host (DSL_SPEC §7) — and respect field constraints (`immutable`, `required`, `unique`) enforced by the store; bulk updates are subject to row quotas. The same holds for `delete`.
...
| D15 | Delete | **Resolved 2026-09-21** | `r.delete()` / `.where(...).delete()`; destroys the record and its history, because the keep-it case is a field the author marks and filters on (the projects solution's `expired`). Alternatives weighed and dropped: a parallel DELETED record type per type (a record type defines shape and behaviour, not location — and nothing retypes a record), one DELETED type for everything (fields are per type; a heterogeneous bucket has no schema to read back through), and a platform-owned live/deleted flag with automatic filtering (takes a decision belonging to the author). The audit lives on the record the deleting activity was anchored to, never on what it destroyed. Refused while another record still points at it — checked across everything the script deleted, once, at its end (at commit when staged; when the script finishes when written through), so deleting a subtree in one run is allowed while deleting a referenced parent alone is not. Open: whether reporting rows are purged with it. No environment gate — "no deletes in production" was ruled and reversed the same day; publishing governs the SDM and pages, not data. |
