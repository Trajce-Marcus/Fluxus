...
The package carries a small model in [config/](config/) — shared pools (`attributes.json`, `functions.json`) plus one file per entity (record type + workflow) under `config/entities/`, merged into one `SolutionConfig` by [src/config.ts](src/config.ts). **It is what the tests run against, and nothing else.** Nothing installs it into a database (the seed script and every other prepopulation path went on 2026-08-05), and it is not what the running app loads: the app fetches config from the server. The model is config; the runtime is generic.

It was cut down to that on 2026-08-11. Until 2026-08-05 the seed script pushed these files into a database, so a real solution could descend from them and then drift away as files and database were edited separately — leaving what looked like two copies of one model. Everything no test reached was deleted (seven entities, twenty-eight attributes), so what remains cannot be mistaken for a solution: nine entities, reached by `test/dsl-wiring.test.ts` here and by the server's `test/headless.test.ts`.
