...
**What is open, in the URL (2026-08-11):** `?page=<pageId>&record=<recordId>` beside `?operation=`. `page` is the page's id, which is its stored path; `record` names the record a page about *many* instances opened on (a one-instance page finds or creates its own, a pure view has none). Both are read at boot, so a link opens where it says; picking another page rewrites `page` and drops `record`, which belonged to the page it was opened on. **Each page opened is a history entry** (2026-09-23): a menu pick or a page opened from a page pushes one — re-opening what is already open pushes none — and Back/Forward reopen what the URL then names. The entry was replaced until then, so Back had nothing inside the app to return to. The operation param is a different thing — it establishes *which operation this app runs* before any of this, and it is the client-trust operation handle that will eventually retire it, not this addressing.
