...
import { createEngine, MemoryAdapter, buildGeoModule } from '@fluxus/engine';
...
for (const org of orgs) {
  for (const sol of await listSolutions(db, org.id)) {
    ...
    try {
      ...
      const engine = createEngine({ store: adapter, config, services: [buildNotifyModule(consoleNotifySink), buildGeoModule(adapter)] });
