...
// engine against the real config and P26-011's real WBS/CBS — inside one
// transaction that is rolled back at the end, so nothing persists. Re-checks the four defects §10a measured
...
const run = async (id: string, attrs: Record<string, unknown>, anchorId: string | null, options?: { acknowledgedWarnings?: boolean }) => {
  const anchor = anchorId ? await host.store.getRecord(anchorId) : null;
  try {
    const result = await host.engine.runActivity(act(id), attrs, anchor as never, options);
...
const ask = async (id: string, attrs: Record<string, unknown>, anchorId: string | null) =>
  (await host.engine.runQuery(act(id), attrs, anchorId ? await host.store.getRecord(anchorId) as never : null)).data;
const fields = async (id: string) => (await host.store.getRecord(id))?.customFields as Record<string, unknown> | undefined;
const allOf = async (typeRef: string, filter: (f: Record<string, unknown>) => boolean) =>
  (await host.store.getRecordTypeData(typeRef)).filter((r) => filter(r.customFields));

// Find real WBS leaves / CBS codes on P26-011 to test against.
const wbsRows = await ask('act_list_wbs_nodes', { project_id: PROJECT }, PROJECT) as { id: string; code: string }[];
...
if (!wbs34 || !wbs35) { console.log('cannot continue without WBS nodes'); await host.store.rollback(); await closeDb(db); process.exit(1); }

const cbsRows = await ask('act_list_cbs_nodes', { project_id: PROJECT }, PROJECT) as { id: string; code: string }[];
...
if (!cbs1300 || !cbs4200) { console.log('cannot continue without CBS codes'); await host.store.rollback(); await closeDb(db); process.exit(1); }
...
const wgCreated = await run('act_create_work_groups', { project_id: PROJECT, wg_parent: '', wg_code: 'WG-TEST', name: 'Test crew', manager: 'Hughie', wg_type: 'Crew' }, null);
check('create a work group', wgCreated.ok, wgCreated.ok ? '' : wgCreated.why);
const wgId = wgCreated.ok ? wgCreated.result.recordId! : '';

const wgParentCreated = await run('act_create_work_groups', { project_id: PROJECT, wg_parent: '', wg_code: 'WG-TEST-SPREAD', name: 'Test spread', manager: 'Owner', wg_type: 'Crew' }, null);
...
const wgChildCreated = await run('act_create_work_groups', { project_id: PROJECT, wg_parent: wgParentId, wg_code: 'WG-TEST-CHILD', name: 'Test child', manager: 'Child mgr', wg_type: 'Crew' }, null);
...
const wgGrandchild = await run('act_create_work_groups', { project_id: PROJECT, wg_parent: wgChildId, wg_code: 'WG-TEST-GRANDCHILD', name: 'Test grandchild', manager: 'X', wg_type: 'Crew' }, null);
...
const expireParentTooSoon = await run('act_delete_work_groups', { expired: 'true' }, wgParentId);
check('expiring a parent with an unexpired child is refused', !expireParentTooSoon.ok, expireParentTooSoon.ok ? 'it was allowed' : '');
const expireChild = await run('act_delete_work_groups', { expired: 'true' }, wgChildId);
check('expire the child first', expireChild.ok, expireChild.ok ? '' : expireChild.why);
const expireParentNow = await run('act_delete_work_groups', { expired: 'true' }, wgParentId);
check('now the parent expires (its only child is already expired)', expireParentNow.ok, expireParentNow.ok ? '' : expireParentNow.why);

const wgDup = await run('act_create_work_groups', { project_id: PROJECT, wg_parent: '', wg_code: 'WG-TEST', name: 'Duplicate', manager: 'X', wg_type: 'Crew' }, null);
check('a duplicate wg_code on the same project is refused', !wgDup.ok, wgDup.ok ? 'it was allowed' : '');

console.log('\n── resources ─────────────────────────────────────────────────────────');
const welder = await run('act_create_resources', { project_id: PROJECT, code: 'TEST-WELD', description: 'Test welder', res_type: 'LAB', unit: 'hr', rate: '95', cbs_id: cbs1300.id }, null);
check('create a resource (welder, $95/hr)', welder.ok, welder.ok ? '' : welder.why);
const welderId = welder.ok ? welder.result.recordId! : '';

const sideboom = await run('act_create_resources', { project_id: PROJECT, code: 'TEST-SIDE', description: 'Test sideboom', res_type: 'PLT', unit: 'hr', rate: '310', cbs_id: cbs4200.id }, null);
...
console.log('\n── standard set ──────────────────────────────────────────────────────');
const stdWelders = await run('act_create_wg_resources', { project_id: PROJECT, wg_id: wgId, resource_id: welderId, quantity: '6' }, null);
check('standard set: 6 welders', stdWelders.ok, stdWelders.ok ? '' : stdWelders.why);
const stdSidebooms = await run('act_create_wg_resources', { project_id: PROJECT, wg_id: wgId, resource_id: sideboomId, quantity: '2' }, null);
check('standard set: 2 sidebooms', stdSidebooms.ok, stdSidebooms.ok ? '' : stdSidebooms.why);

console.log('\n── shift report, seeded lines, work_hours ───────────────────────────');
const report = await run('act_create_shift_reports', {
...
const reportRec = await fields(reportId);
...
const lines = await ask('act_list_shift_report_resources', { report_id: reportId }, reportId) as { id: string; resource_id: string; quantity: number; rate: number; cbs_id: string }[];
...
const w1 = await run('act_create_shift_wbs', { project_id: PROJECT, report_id: reportId, wbs_id: wbs34.id, hours: '0.1', qty_completed: '1' }, null);
const w2 = await run('act_create_shift_wbs', { project_id: PROJECT, report_id: reportId, wbs_id: wbs34.id, hours: '8.2', qty_completed: '2' }, null);
const w3 = await run('act_create_shift_wbs', { project_id: PROJECT, report_id: reportId, wbs_id: wbs35.id, hours: '1.7', qty_completed: '3' }, null);
check('three WBS rows added (0.1 + 8.2 + 1.7)', w1.ok && w2.ok && w3.ok, [w1, w2, w3].map((r) => r.ok ? '' : r.why).join(' '));

const calc = await run('act_calculate_shift_reports', {}, reportId);
check('Calculate accepts 0.1+8.2+1.7 against work_hours=10 (float-safe)', calc.ok, calc.ok ? '' : calc.why);

const pricedLines = await ask('act_list_shift_report_resources', { report_id: reportId }, reportId) as { id: string; quantity: number; rate: number; tracked_cost: number; calculated: string }[];
...
const adjust = await run('act_modify_shift_report_resource', { quantity: '61' }, welderLine!.id);
check('adjusting a line after Calculate is allowed', adjust.ok, adjust.ok ? '' : adjust.why);
const submitTooSoon = await run('act_submit_shift_reports', {}, reportId);
check('Submit refused: a line changed since the last Calculate', !submitTooSoon.ok, submitTooSoon.ok ? 'it was allowed' : '');
const recalc = await run('act_calculate_shift_reports', {}, reportId);
check('Calculate re-run after the adjustment', recalc.ok, recalc.ok ? '' : recalc.why);
const submitted = await run('act_submit_shift_reports', {}, reportId);
check('Submit now accepted', submitted.ok, submitted.ok ? '' : submitted.why);
check('status is Submitted', (await fields(reportId))?.status === 'Submitted');

console.log('\n── approve: largest-remainder division ──────────────────────────────');
// welder line is now 61 × 95 = 5795 (rate stayed the same, quantity moved).
const approved = await run('act_approve_shift_reports', {}, reportId);
check('Approve runs', approved.ok, approved.ok ? '' : approved.why);
check('status is Approved, approved_by/date set', (await (async () => {
  const f = await fields(reportId);
  return f?.status === 'Approved' && !!f?.approved_by && !!f?.approved_date;
})()));

const usageRows = await allOf('rt_wbs_resource_usage', (f) => f.report_id === reportId);
...
const report2 = await run('act_create_shift_reports', {
  project_id: PROJECT, wg_id: wgId,
...
check('work_hours crosses midnight correctly: 12', Number((await fields(report2Id))?.work_hours) === 12, `work_hours=${(await fields(report2Id))?.work_hours}`);

await run('act_create_shift_wbs', { project_id: PROJECT, report_id: report2Id, wbs_id: wbs34.id, hours: '6' }, null);
await run('act_create_shift_wbs', { project_id: PROJECT, report_id: report2Id, wbs_id: wbs35.id, hours: '6' }, null);
const calc2 = await run('act_calculate_shift_reports', {}, report2Id);
check('Calculate on the second report', calc2.ok, calc2.ok ? '' : calc2.why);
await run('act_submit_shift_reports', {}, report2Id);
const approved2 = await run('act_approve_shift_reports', {}, report2Id);
check('Approve runs', approved2.ok, approved2.ok ? '' : approved2.why);
const usage2 = await allOf('rt_wbs_resource_usage', (f) => f.report_id === report2Id);
...
const reportWarn = await run('act_create_shift_reports', {
  project_id: PROJECT, wg_id: wgId,
  report_date: '2026-09-23', shift: 'Day', start_time: '06:00', end_time: '16:00', break_hours: '0',
}, null);
...
const wbsWarnRow = await run('act_create_shift_wbs', { project_id: PROJECT, report_id: reportWarnId, wbs_id: wbs34.id, hours: '10' }, null);
...
const calcWarn = await run('act_calculate_shift_reports', {}, reportWarnId);
check('Calculate accepts the matching total', calcWarn.ok, calcWarn.ok ? '' : calcWarn.why);

await run('act_modify_shift_wbs', { hours: '8' }, wbsWarnRowId);
const submitWarn = await run('act_submit_shift_reports', {}, reportWarnId);
...
const submitAck = await run('act_submit_shift_reports', {}, reportWarnId, { acknowledgedWarnings: true });
check('acknowledging the warning is a legitimate answer — Submit goes through', submitAck.ok, submitAck.ok ? '' : submitAck.why);

const approveWarn = await run('act_approve_shift_reports', {}, reportWarnId);
...
const approveAck = await run('act_approve_shift_reports', {}, reportWarnId, { acknowledgedWarnings: true });
...
const reportRC = await run('act_create_shift_reports', {
  project_id: PROJECT, wg_id: wgId,
  report_date: '2026-09-23', shift: 'Day', start_time: '06:00', end_time: '16:00', break_hours: '0',
}, null);
const reportRCId = reportRC.ok ? reportRC.result.recordId! : '';
await run('act_create_shift_wbs', { project_id: PROJECT, report_id: reportRCId, wbs_id: wbs34.id, hours: '10' }, null);
await run('act_calculate_shift_reports', {}, reportRCId);
const submitRC = await run('act_submit_shift_reports', {}, reportRCId);
check('submit a clean report (no warning, hours match)', submitRC.ok, submitRC.ok ? '' : submitRC.why);

const rejected = await run('act_reject_shift_reports', {}, reportRCId);
check('Reject sends it back to Draft', rejected.ok && (await fields(reportRCId))?.status === 'Draft', rejected.ok ? '' : rejected.why);
check('nothing posted to the ledger by a reject', (await allOf('rt_wbs_resource_usage', (f) => f.report_id === reportRCId)).length === 0);

console.log('\n── §5.3: Cancel expires a Draft report — §6 reads it as nothing filed ─');
const cancelled = await run('act_cancel_shift_reports', { expired: 'true' }, reportRCId);
check('Cancel expires the (rejected, now draft) report', cancelled.ok && (await fields(reportRCId))?.expired === 'true', cancelled.ok ? '' : cancelled.why);
const listAfterCancel = await ask('act_list_shift_reports', { project_id: PROJECT }, PROJECT) as { id: string }[];
...
const amendmentWithNoParent = await run('act_create_shift_report_amendments', { project_id: PROJECT, wg_id: wgId, amended_report_id: '' }, null);
...
const amendment = await run('act_create_shift_report_amendments', { project_id: PROJECT, wg_id: wgId, amended_report_id: reportId }, null);
...
check('the amendment carries amended_report_id, project_id and wg_id — sourced, not asked',
  (await fields(amendmentId))?.amended_report_id === reportId
  && (await fields(amendmentId))?.project_id === PROJECT
  && (await fields(amendmentId))?.wg_id === wgId);
check('date, shift, start and end time are never asked — they sit blank',
  (await fields(amendmentId))?.report_date === '' && (await fields(amendmentId))?.start_time === '' && (await fields(amendmentId))?.end_time === '');
check('report_no assigned from the shared sequence', /^SR-\d{4}$/.test(String((await fields(amendmentId))?.report_no ?? '')));

const amendOnDraft = await run('act_create_shift_report_amendments', { project_id: PROJECT, wg_id: wgId, amended_report_id: reportRCId }, null);
check('an amendment cannot be raised against a report that is not approved', !amendOnDraft.ok, amendOnDraft.ok ? 'it was allowed' : '');

const notes = await run('act_edit_shift_report_amendment_notes', { site_notes: 'Welder hours overstated on the original report.' }, amendmentId);
...
const amendLine = await run('act_add_shift_report_amendment_line', {
...
check('the line prices itself immediately — no Calculate for an amendment', (await (async () => {
  const f = await fields(amendLineId);
  return Number(f?.rate) === 95 && Number(f?.tracked_cost) === -380;
})()), JSON.stringify(await fields(amendLineId)));
...
const amendLineOnOrdinary = await run('act_add_shift_report_amendment_line', {
...
const ordinaryLineOnAmendment = await run('act_add_shift_report_resource', {
...
// generally.
const rcLines = await allOf('rt_shift_report_resource_usage', (f) => f.report_id === reportRCId);
...
const adjustOrdinaryLineOrdinary = await run('act_modify_shift_report_resource', { quantity: '99' }, rcWelderLine!.id);
...
const adjustOrdinaryLineOnAmendment = await run('act_adjust_shift_report_amendment_line', { quantity: '-5' }, rcWelderLine!.id);
...
const modifyOrdinaryReport = await run('act_modify_shift_reports', { report_date: '2026-09-23', shift: 'Day', start_time: '06:00', end_time: '16:00', break_hours: '0' }, reportRCId);
...
const modifyAmendmentAsReport = await run('act_modify_shift_reports', { report_date: '2026-09-23', shift: 'Day', start_time: '07:00', end_time: '15:00' }, amendmentId);
...
const editNotesOnOrdinary = await run('act_edit_shift_report_amendment_notes', { site_notes: 'should not land here' }, reportRCId);
...
const wbsRowOnAmendment = await run('act_create_shift_wbs', { project_id: PROJECT, report_id: amendmentId, wbs_id: wbs34.id, hours: '4' }, null);
...
// photos, no defects."
const defectOnAmendment = await run('act_create_defects', {
...
const calcOnAmendment = await run('act_calculate_shift_reports', {}, amendmentId);
...
const amendSubmit = await run('act_submit_shift_reports', {}, amendmentId);
...
const amendApprove = await run('act_approve_shift_reports', {}, amendmentId);
...
const amendUsage = await allOf('rt_wbs_resource_usage', (f) => f.report_id === amendmentId);
...
const listWithAmendment = await ask('act_list_shift_reports', { project_id: PROJECT }, PROJECT) as { id: string }[];
check('the amendment does not appear in the project\'s shift-report list', !listWithAmendment.some((r) => r.id === amendmentId));
const groupListWithAmendment = await ask('act_list_work_group_reports', { wg_id: wgId }, PROJECT) as { id: string }[];
check('nor in the work group\'s own list', !groupListWithAmendment.some((r) => r.id === amendmentId));

console.log('\n── defects ───────────────────────────────────────────────────────────');
const defectOnApproved = await run('act_create_defects', {
...
const report3 = await run('act_create_shift_reports', {
  project_id: PROJECT, wg_id: wgId,
...
const defect = await run('act_create_defects', {
...
check('defect_no assigned, DEF- prefixed, status Open', (await (async () => {
  const f = await fields(defectId);
  return /^DEF-\d{3}$/.test(String(f?.defect_no)) && f?.status === 'Open';
})()), JSON.stringify(await fields(defectId)));

const rectified = await run('act_rectify_defects', { rectified_date: '2026-09-24', rectification_notes: 'Recoated' }, defectId);
check('rectify the defect', rectified.ok, rectified.ok ? '' : rectified.why);
check('status is Rectified', (await fields(defectId))?.status === 'Rectified');
const verified = await run('act_verify_defects', { verified_date: '2026-09-25', verified_by: 'QA' }, defectId);
check('verify (close) the defect', verified.ok, verified.ok ? '' : verified.why);
check('status is Closed', (await fields(defectId))?.status === 'Closed');
...
const allProjectDefects = await ask('act_list_defects', { project_id: PROJECT }, PROJECT) as { id: string }[];
...
const reportOnlyDefects = await ask('act_list_defects', { project_id: PROJECT, report_id: report3Id }, PROJECT) as { id: string }[];
...
const expected = await ask('act_list_expected_work_groups', { project_id: PROJECT }, PROJECT) as { wg_code: string }[];
...
const cbsTotal = await ask('act_total_cbs_nodes', { project_id: PROJECT }, PROJECT);
check('CBS total is leaf-only and matches the approved budget', Number(cbsTotal) === 40_620_000, `total=${cbsTotal}`);

console.log(`\n${failures === 0 ? 'all checks passed' : `${failures} FAILURE(S)`} — nothing persisted (rolled back)`);
