...
// real engine against the real model and the real records — and **never calls
// `writeBack`**, so nothing persists. That is what makes it safe to approve a
// WBS here: the freeze happens in memory, is checked, and is thrown away.
//
...
const rec = (id: string) => host.adapter.getRecord(id);
...
const run = (id: string, attrs: Record<string, unknown>, anchor: string | null) => {
  try {
    const result = host.engine.runActivity(act(id), attrs, anchor ? rec(anchor) : null);
...
const ask = (id: string, attrs: Record<string, unknown>) => host.engine.runQuery(act(id), attrs, rec(PROJECT)).data;

console.log(`\n── totals, before anything is entered ───────────────────────────────`);
const cbsTotal = ask('act_total_cbs_nodes', { project_id: PROJECT });
const wbsTotal0 = ask('act_total_wbs_baseline', { project_id: PROJECT });
...
const baselined = run('act_baseline_wbs_nodes', {
...
const leaf = rec('1.1')?.customFields as Record<string, unknown>;
...
const onParent = run('act_baseline_wbs_nodes', { baseline_budget: '999' }, '1.0');
check('a parent is refused a baseline', !onParent.ok, onParent.ok ? 'it was allowed' : '');

const wbsTotal1 = ask('act_total_wbs_baseline', { project_id: PROJECT });
...
const added = run('act_create_wbs_nodes', {
...
const wasLeaf = rec('1.1')?.customFields as Record<string, unknown>;
...
check('and the total falls back to zero', Number(ask('act_total_wbs_baseline', { project_id: PROJECT })) === 0);

console.log(`\n── delete, from the bottom ──────────────────────────────────────────`);
const delParent = run('act_delete_wbs_nodes', { expired: 'true' }, '1.0');
check('a node with children is refused', !delParent.ok, delParent.ok ? 'it was allowed' : '');
const delLeaf = run('act_delete_wbs_nodes', { expired: 'true' }, '5.3');
check('a childless node goes', delLeaf.ok, delLeaf.ok ? '' : delLeaf.why);

console.log(`\n── approval, and the freeze ─────────────────────────────────────────`);
const tooEarly = run('act_activate_projects', {}, PROJECT);
check('a project cannot start before the WBS is approved', !tooEarly.ok, tooEarly.ok ? 'it started' : '');
const approved = run('act_approve_wbs_projects', {}, PROJECT);
check('the WBS is approved', approved.ok, approved.ok ? '' : approved.why);
check('  …and the project says so', (rec(PROJECT)?.customFields as Record<string, unknown>)?.wbs_status === 'approved');
...
for (const [what, id, attrs, anchor] of [
  ...
  const attempt = run(id, attrs, anchor);
  check(`a frozen WBS refuses ${what}`, !attempt.ok, attempt.ok ? 'it was allowed' : '');
}
const newNode = run('act_create_wbs_nodes', { wbs_project: PROJECT, wbs_parent: '', code: '6.0', name: 'Late idea' }, null);
check('a frozen WBS refuses a new node', !newNode.ok, newNode.ok ? 'it was allowed' : '');

const forecast = run('act_forecast_wbs_nodes', { forecast_cost: '300000' }, '1.2');
check('but the forecast stays open', forecast.ok, forecast.ok ? '' : forecast.why);
known('  …and the forecast total should move', NO_CAST, ask('act_total_wbs_forecast', { project_id: PROJECT }));
...
const started = run('act_activate_projects', {}, PROJECT);
check('start is allowed once the WBS is approved', started.ok, started.ok ? '' : started.why);
check('  …and the status says Active', (rec(PROJECT)?.customFields as Record<string, unknown>)?.status === 'Active');
const completed = run('act_complete_projects', {}, PROJECT);
check('complete is allowed from Active', completed.ok, completed.ok ? '' : completed.why);
check('  …and the status says so', (rec(PROJECT)?.customFields as Record<string, unknown>)?.status === 'Completed');

console.log(`\n${failures === 0 ? 'all checks passed' : `${failures} FAILED`} — nothing written (no writeBack)\n`);
await closeDb(db);
